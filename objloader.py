...
def MTL(dir, filename):
    contents = {}
    mtl = None
    for line in open(dir + filename, "r",encoding='UTF-8'):
        ...
        if values[0] == 'newmtl':
            ...
            # load the texture referred to by this declaration
            mtl[values[0]] = dir + values[1]
            surf = pygame.image.load(mtl['map_Bump'])
            image = pygame.image.tostring(surf, 'RGBA', 1)
            ix, iy = surf.get_rect().size
            texid = mtl['texture_Bump'] = glGenTextures(1)
            glBindTexture(GL_TEXTURE_2D, texid)
            glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER,
                GL_LINEAR)
            glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER,
            ...
            # load the texture referred to by this declaration
            mtl[values[0]] = dir + values[1]
            surf = pygame.image.load(mtl['map_D'])
            image = pygame.image.tostring(surf, 'RGBA', 1)
            ix, iy = surf.get_rect().size
            texid = mtl['texture_D'] = glGenTextures(1)
            glBindTexture(GL_TEXTURE_2D, texid)
            glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER,
                GL_LINEAR)
            glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER,
